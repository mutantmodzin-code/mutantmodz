...
import { 
  ...
  Bike, Zap, Wrench, Shirt, Briefcase, Shield, Package, Calendar, Flame,
...
export default function Navigation({ currentPage, onNavigate }: NavigationProps) {
  ...
  return (
    <>
      <nav 
        className={`fixed top-0 w-full z-50 transition-all duration-500 cubic-bezier(0.25, 1, 0.5, 1) border-b border-zinc-800/80 ${
          isScrolled 
      ...
      >
        ...
        <div className={`transition-all duration-500 ease-out border-b border-zinc-800/40 ${isScrolled ? 'py-1' : 'py-4'}`}>
              ...
              <div className="hidden lg:flex flex-1 max-w-4xl mx-auto relative group z-10">
                <div className={`flex w-full bg-zinc-900/50 backdrop-blur-sm border border-zinc-700/60 rounded-full transition-all duration-400 shadow-inner group-focus-within:bg-zinc-900/80 group-focus-within:border-red-500/50 group-focus-within:shadow-[0_0_20px_rgba(220,38,38,0.15)] ${isScrolled ? 'h-10' : 'h-12'}`}>
                  ...
                  <div className="relative flex items-center h-full border-r border-zinc-700/60 transition-colors group-focus-within:border-zinc-600">
                    <button 
                      className="flex items-center gap-1.5 px-4 md:px-5 text-sm font-medium text-zinc-300 hover:text-white transition-colors"
                  ...
                  <div className="relative flex-1 flex items-center h-full">
                    <input 
                      type="text" 
                      placeholder="Search helmets, riding gear, luggage, accessories..." 
                      className="w-full h-full bg-transparent text-white text-[15px] pl-4 pr-12 focus:outline-none placeholder-zinc-500 font-light transition-all duration-300"
                    />
                ...
                {categoriesData.map((cat) => (
                  ...
                  >
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-4 text-[12px] xl:text-[13px] font-bold tracking-wide uppercase transition-colors duration-300 relative ${activeDropdown === cat.id ? 'text-white' : 'text-zinc-300 hover:text-white'}`}
                    >
                      <cat.icon size={16} className={`transition-all duration-300 group-hover:scale-110 group-hover:text-red-500 ${activeDropdown === cat.id ? 'text-red-500' : 'text-zinc-500'}`} />
                      {cat.label}
                      {/* Smooth Underline Animation */}
                      <span className={`absolute left-1/2 -translate-x-1/2 bottom-[2px] h-[2px] bg-red-600 transition-all duration-400 ease-out shadow-[0_0_10px_rgba(220,38,38,0.8)] rounded-full ${activeDropdown === cat.id ? 'w-[80%]' : 'w-0 group-hover:w-[80%]'}`}></span>
                    </button>

                    {/* Dynamic Mega Menu Dropdown */}
                    <div className={`absolute left-0 top-full w-full bg-gradient-to-b from-zinc-900 via-zinc-950 to-black border-y border-zinc-800/80 shadow-[0_40px_80px_rgba(0,0,0,0.95)] transition-all duration-500 cubic-bezier(0.16, 1, 0.3, 1) transform origin-top overflow-hidden ${
                      activeDropdown === cat.id ? 'opacity-100 scale-y-100 translate-y-0 visible h-auto pb-4 max-h-[500px]' : 'opacity-0 scale-y-95 -translate-y-4 invisible h-0 pointer-events-none'
                    }`}>
                      <div className="max-w-[1500px] mx-auto px-8 py-10 flex gap-12">
                                 ...
                                 {col.links.map(link => (
                                   <li key={link}>
                                     <button 
                                      ...
                                      className="text-zinc-400 hover:text-red-400 text-[14px] font-medium transition-all duration-300 hover:translate-x-2 flex items-center gap-2 group/link"
                                     >
                                       <ChevronRight size={14} className="opacity-0 -ml-4 transition-all duration-300 group-hover/link:opacity-100 group-hover/link:ml-0 text-red-500" /> 
                                       {link}
                                     </button>
                                   </li>
                                 ))}
                         ...
                         <div className="w-[30%] grid gap-5">
                            ...
                            {cat.featured.map((card, idx) => (
                              <div key={idx} className="group/card relative cursor-pointer rounded-2xl overflow-hidden hover:shadow-[0_15px_40px_rgba(220,38,38,0.2)] transition-all duration-500 h-48 border border-zinc-800/50 hover:border-red-500/50" onClick={() => handleNavClick('products')}>
        ...
        <div className={`lg:hidden fixed inset-0 top-[100px] z-40 bg-zinc-950/98 backdrop-blur-2xl transition-all duration-500 overflow-y-auto ease-in-out ${
