...
export default function Products({ onNavigate }: ProductsProps) {
  ...
  const [allProducts, setAllProducts] = useState<{ regular: Product[], combos: Product[], garage: Product[] }>({
    regular: [], combos: [], garage: []
  });
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        ...
        // Initial products state for stats calculation
  ...
  const filteredProducts = useMemo(() => {
    const isGarageTypeSelected = 
      ['garage-sale', 'garage sale', 'offer'].includes(selectedCategory.toLowerCase()) ||
      selectedTypes.some(t => t.toLowerCase() === 'garage sale' || t.toLowerCase() === 'garage-sale');

    // 1. Determine the base pool of products
    let basePool;

    if (isGarageTypeSelected) {
        // If Garage Sale is selected/active, we want to show the garage versions for items that have them, 
        // plus all other garage sale items, and regular items that don't have garage versions.
        const garageSkus = new Set(allProducts.garage.map(p => p.sku).filter(Boolean));
        
        basePool = [
            ...allProducts.regular.filter(p => !p.sku || !garageSkus.has(p.sku)),
            ...allProducts.combos,
            ...allProducts.garage
        ];
    } else {
        // DEFAULT: Show only inventory products (regular + combos)
        // Strictly exclude garage items from the pool unless explicitly requested
        basePool = [...allProducts.regular, ...allProducts.combos];
    }
    ...
    return basePool.filter(p => {
      ...
      const matchesType = selectedTypes.length === 0 || 
        selectedTypes.some(t => {
          const type = t.toLowerCase().replace(/-/g, ' ');
          return (p.sub_category_type?.toLowerCase().replace(/-/g, ' ') === type) ||
                 (p.category_name?.toLowerCase().replace(/-/g, ' ') === type) ||
                 (p.category?.toLowerCase().replace(/-/g, ' ') === type);
        });
  ...
  }, [allProducts, selectedCategory, searchQuery, selectedBrands, selectedTypes, priceRange, availabilityFilter]);
              ...
              <div className="space-y-10">
                ...
                <div className="space-y-4">
                  ...
                  {expandedSections.includes('product-type') && (
                    <div className="space-y-3 pt-2">
                      {/* Permanent Garage Sale Filter Option */}
                      <label className="flex items-center justify-between group cursor-pointer">
                        <div className="flex items-center gap-3">
                          <input type="checkbox" checked={selectedTypes.includes('garage sale')} onChange={() => setSelectedTypes(prev => prev.includes('garage sale') ? prev.filter(t => t !== 'garage sale') : [...prev, 'garage sale'])} className="hidden" />
                          <div className={cn("w-4 h-4 border-2 rounded transition-all flex items-center justify-center", selectedTypes.includes('garage sale') ? "bg-red-600 border-red-600" : "border-white/10 group-hover:border-white/30")}>
                            {selectedTypes.includes('garage sale') && <span className="text-[8px] text-white">✓</span>}
                          </div>
                          <span className={cn("text-[11px] font-bold uppercase tracking-widest transition-colors", selectedTypes.includes('garage sale') ? "text-white" : "text-zinc-500 group-hover:text-zinc-300")}>Garage Sale</span>
                        </div>
                        <span className="text-zinc-700 text-[10px] font-black">({allProducts.garage.length})</span>
                      </label>

                      {Object.entries(availableFilters.types)
                        .filter(([type]) => type.toLowerCase() !== 'garage sale' && type.toLowerCase() !== 'garage-sale')
                        .map(([type, count]) => (
                  ...
                  <div className={cn("absolute top-0 left-0 h-full w-[85%] bg-zinc-950 border-r border-white/5 flex flex-col transition-transform duration-500", showSortMenu ? "translate-x-0" : "-translate-x-full")}>
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar p-8 pt-0 space-y-10">
                      ...
                      <div className="space-y-4">
                        ...
                        {expandedSections.includes('product-type') && (
                          <div className="space-y-3 pt-2">
                            {/* Mobile Permanent Garage Sale Filter Option */}
                            <label className="flex items-center justify-between group cursor-pointer">
                              <div className="flex items-center gap-3">
                                <input type="checkbox" checked={selectedTypes.includes('garage sale')} onChange={() => setSelectedTypes(prev => prev.includes('garage sale') ? prev.filter(t => t !== 'garage sale') : [...prev, 'garage sale'])} className="hidden" />
                                <div className={cn("w-4 h-4 border-2 rounded transition-all flex items-center justify-center", selectedTypes.includes('garage sale') ? "bg-red-600 border-red-600" : "border-white/10")}>
                                  {selectedTypes.includes('garage sale') && <span className="text-[8px] text-white">✓</span>}
                                </div>
                                <span className={cn("text-[11px] font-bold uppercase tracking-widest transition-colors", selectedTypes.includes('garage sale') ? "text-white" : "text-zinc-500")}>Garage Sale</span>
                              </div>
                              <span className="text-zinc-700 text-[10px] font-black">({allProducts.garage.length})</span>
                            </label>
